...
c      endif
  100 CALL LEPTO
      IF(LST(21).NE.0) THEN
        ...
        GOTO 100
      ENDIF
      IF(.not. check_pid(partpid) .or. .not. look_for_pions()) THEN ! partpid is input PID, could be 211, -211, 2212, 223, 211, etc
         GOTO 100
      ENDIF
 ...
 200  CONTINUE
      ...
      logical function look_for_pions()
      COMMON/LUJETS/N,K(4000,5),P(4000,5),V(4000,5)
      DOUBLE PRECISION P,V
      INTEGER N,K,Neta,Npid
      look_for_pions=.FALSE.
      Neta = 0
      DO 300 Npid=1,N
      IF ((K(NPID,2).EQ.211 .OR. K(NPID,2).EQ.-211) .AND. K(NPID,3).NE.0) THEN ! check if particle selected is a charged pion
         look_for_pions=.TRUE.
      ENDIF
 300  CONTINUE
